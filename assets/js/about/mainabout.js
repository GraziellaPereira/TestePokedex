// declaração de variáveis que armazenam referências a elementos HTML com base em seus IDs
const aboutValues = document.getElementById('about-values')
const baseStats = document.getElementById('base-stats')
const evolutionValues = document.getElementById('evolution-values')
const movesValues = document.getElementById('moves-values')

// Conseguir valores dos pokemons
document.addEventListener('DOMContentLoaded', () => { // O bloco de código só será executado quando a DOM poder ser manipulada e toda a estrutura HTML já estiver completamente carregada
    var pokemonDetail = JSON.parse(sessionStorage.getItem('pokemon')) //recupera o item 'pokemon' armazenado na sessão do navegador, transforma a string json de volta em objeto JS
    if (pokemonDetail){ // se o objeto pokemon ter um valor, chamara a função fillValues
        fillValues(pokemonDetail)
        document.body.classList.add(pokemonDetail.type) // para adicionar estilos específicos para o pokemon conforme seu tipo
    }
});
